...
export const POST = withAuth(
  async (req: NextRequest, { school }: AuthContext): Promise<NextResponse> => {
    try {
      if (!school) return badRequest("School context required");
      const { name, arm, section, level, classTeacher } = await req.json();
      if (!name || !level) return badRequest("Class name and level are required");
      const classSection = arm || section || null;

      const id = generateId();
      await execute(
        `INSERT INTO classes (id, name, section, level, class_teacher_id, school_id, academic_session, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
        [id, name, classSection, level, classTeacher || null, school.id, school.academic_session]
      );

      return created({ id, name, section: classSection, level, school_id: school.id });
    } catch (err) {
      return serverError(err);
    }
  },
